...
data<- read_csv("data/covariates_installation.csv")

# Contraceptive prevalence: Any method (Percent)
CPAnyP <- data$CPAnyP
...
UNMP <- data$UNMP
# "Unmet need for family planning: Any modern method (Percent)"  
UNMModP <- data$UNMModP
# "Total demand for family planning (Percent)"    
DEMTot <- data$DEMTot
# "Demand for family planning satisfied by any modern method (Percent)"
DEMMod <- data$DEMMod
# "Contraceptive prevalence: Any method (Number)"          
CPAnyN <- data$CPAnyN
# "Demand for family planning satisfied by any method (Percent)" 
DEMAny <- data$DEMAny
...
UNMModN <- data$UNMModN
# "Contraceptive prevalence: Any traditional method (Number)" 
CPTradN <- data$CPTradN
# "Total demand for family planning (Number)" 
DEMTotN <- data$DEMTotN
# Total Fertility Rate
tfr <- data$tfr
# Total number of Women
popFT <- data$popFT
ratiof <- (data$popFage/data$totpop)*100
# ITU data 
itu_internet <- data$itu_internet
itu_mobile <- data$itu_mobile
itu_mobile_f <- data$itu_female_m
# installations
...
region_Northern_America <- data$`region_Northern America`
...
model {
  ...
  ## Likelihood Negative Binomial
    for(j in 1:133){
      ...
      p[j] <- r/(r+mu[j]) 
      log(mu[j]) <- delta0 + delta1 * CPModP[j] +   delta2 * CPTrad[j] + delta3 * UNMP[j] + delta4 * tfr[j] + delta5 * popFage[j] + delta6 * itu_internet[j] + delta7 * itu_mobile[j] +
          
          # Regions
          
          # Africa
          
          delta8 * region_Northern_Africa[j] +
          delta9 * region_Sub_Saharan_Africa[j] +
          
          # Americas
          
          delta10 * region_Latin_America_Caribbean[j] +
          
          # Asia
          delta11 * region_Central_Asia[j] +
          delta12 * region_Eastern_Asia[j] +
          delta13 * region_South_Eastern_Asia[j] +
          delta14 * region_Southern_Asia[j] +
          delta15 * region_Western_Asia[j] +
          
          # Europe
          delta16 * region_Eastern_EU[j] +
          delta17 * region_Northern_EU[j] +
          delta18 * region_Southern_EU[j] +
          delta19 * region_Western_EU[j] +

          # Oceania

          delta20 * region_Australia_New_Zealand[j] +
          delta21 * region_Melanesia[j] +
          delta22 * region_Polynesia[j] +
          ...
          delta23 * region_Northern_Africa[j] * UNMP[j] +
          delta24 * region_Sub_Saharan_Africa[j] * UNMP[j]+
          
          # Americas
          
          delta25 * region_Latin_America_Caribbean[j] * UNMP[j] +
          
          # Asia
          delta26 * region_Central_Asia[j] * UNMP[j]+
          delta27 * region_Eastern_Asia[j] * UNMP[j] +
          delta28 * region_South_Eastern_Asia[j] * UNMP[j] +
          delta29 * region_Southern_Asia[j] * UNMP[j]+
          delta30 * region_Western_Asia[j] * UNMP[j]+
          
          # Europe
          delta31 * region_Eastern_EU[j] * UNMP[j]+
          delta32 * region_Northern_EU[j] * UNMP[j]+
          delta33 * region_Southern_EU[j] * UNMP[j]+
          delta34 * region_Western_EU[j] * UNMP[j]+

          # Oceania

          delta35 * region_Australia_New_Zealand[j] *UNMP[j]+
          delta36 * region_Melanesia[j] *UNMP[j]+
          delta37 * region_Polynesia[j] *UNMP[j]
...
model.samples <- coda.samples(model.fit, c("yhat", "delta0", "delta1", "delta2", "delta3", "delta4", "delta5", "delta6", "delta7", "delta8", "delta9", "delta10", "delta11", "delta12", 
                                           ...
                                           "delta30", "delta31", "delta32", "delta33", "delta34", "delta35", "delta36", "delta37"), n.iter=iterations, thin=10)
...
d <- model.samples[[1]]
for(i in 2:length(model.samples)){
  d <- rbind(d, model.samples[[i]])
}
d <- data.frame(d)

# sample size
n <- sum(grepl('yhat', names(d)))

# data for plot
plotdat <- data.frame(i=1:n, obs=installation)
...
plotHat <- function(dat, main=NULL, xlab=NULL, ylab=NULL, file=NULL, zoom=1, predCol='mean'){
  ...
  r2 <- round(cor(dat$obs, dat$pred)^2, 3)
  
  if(zoom < 1){
    dat <- dat[dat$obs < quantile(dat$obs,probs=zoom), ]
  }
  
  xlim <- c(0,max(dat$obs))
  ylim <- c(0,max(dat$upper))
...
# make plots
plotHat(plotdat)
plotHat(plotdat, zoom=0.8)

plotHat(plotdat, predCol='median')
plotHat(plotdat, predCol='median', zoom=0.8)
...
delta <- delta %>% mutate(delta = recode(delta, 
                                         ...
                                         delta6="ITU Internet",
                                         delta7="ITU Mobile", 
                                         delta8="Northern Africa",
                                         delta9="Sub Saharan Africa",
                                         delta10="Latin America Caribbean",
                                         delta11="Central Asia", 
                                         delta12="Eastern Asia", 
                                         delta13="South Eastern Asia", 
                                         delta14="Southern Asia",
                                         delta15="Western Asia", 
                                         delta16="Eastern EU", 
                                         delta17="Northern EU",
                                         delta18="Southern EU",
                                         delta19="Western EU", 
                                         delta20="Australia New Zealand", 
                                         delta21="Melanesia",
                                         delta22="Polynesia", 
                                         delta23="UNMP - Northern Africa", 
                                         delta24="UNMP - Sub Saharan Africa",
                                         delta25="UNMP - Latin America Caribbean",
                                         delta26="UNMP - Central Asia",
                                         delta27="UNMP - Eastern Asia",
                                         delta28="UNMP - South Eastern Asia", 
                                         delta29="UNMP - Southern Asia",
                                         delta30="UNMP - Western Asia",
                                         delta31="UNMP - Eastern EU",
                                         delta32="UNMP - Northern EU",
                                         delta33="UNMP - Southern EU",
                                         delta34="UNMP - Western EU",
                                         delta35="UNMP - Australia New Zealand", 
                                         delta36="UNMP - Melanesia", 
                                         delta37="UNMP - Polynesia"))
