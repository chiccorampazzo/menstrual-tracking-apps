...
#---- test pi ----#
d <- model.samples[[1]]
for(i in 2:length(mcmc.list)){d <- rbind(d, model.samples[[i]])}
d <- as.data.frame(d)

hist(d[,'proportion_google[1,1]'])
mean(d[,'proportion_google[1,1]'])
