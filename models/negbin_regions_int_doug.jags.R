model {
  ...
  for(j in 1:123){
    ...
    log(mu[j]) <- delta0 + 
      ...
      #-- Interactions --#
      # Africa
      delta[25] * region_Northern_Africa[j] * UNMP[j] +
      delta[26] * region_Sub_Saharan_Africa[j] * UNMP[j]+
    
      # Americas
      delta[27] * region_Latin_America_Caribbean[j] * UNMP[j] +
      
      # Asia
      delta[28] * region_Central_Asia[j] * UNMP[j]+
      delta[29] * region_Eastern_Asia[j] * UNMP[j] +
      delta[30] * region_South_Eastern_Asia[j] * UNMP[j] +
      delta[31] * region_Southern_Asia[j] * UNMP[j]+
      delta[32] * region_Western_Asia[j] * UNMP[j]+
      
      # Europe
      delta[33] * region_Eastern_EU[j] * UNMP[j]+
      delta[34] * region_Northern_EU[j] * UNMP[j]+
      delta[35] * region_Southern_EU[j] * UNMP[j]+
      delta[36] * region_Western_EU[j] * UNMP[j]+
      
      # Oceania
      delta[37] * region_Australia_New_Zealand[j] *UNMP[j]+
      delta[38] * region_Melanesia[j] *UNMP[j]
